...
f_in _desired= 50e3;
fs = 50e6;
NearestPrime = max(primes(f_in _desired*(NumSamples/2)/fs));
...
%[snr1, enob1, pot_signal _B1, f1, PSD1] = gs_fresp(DAC_Output', NumSamples, fs, f_in, 1);
[Spectrum, f_TS, PSD_OSdB, f_OS, Window] = wall_fresp(DAC_Output, Time, @hann, 1);


% FFT of the DAC output (A)
[DAC_Spectrum, DAC_f _TS, PSD_DAC, DAC_f _OS, DAC_Window] = wall_fresp(DAC_Output, Time, @rectwin, 0);
...
% Voltage signal
[C1_Spectrum, C1_f _TS, PSD_C1, C1_f _OS, C1_Window] = wall_fresp(c1, Time, @rectwin, 0);
% [C2_Spectrum, C2_f _TS, PSD_C2, C2_f _OS, C2_Window] = wall_fresp(c2, Time, @rectwin, 0);
...
num = [R/L 0];
den = [1 R/L 1/(L*C)];
filter_ 1 = tf(num,den);
FCornerFilter = 0;
T = 300;
Filter_TNoise = sqrt(4*k*T*R);

% Filter output
Filter_Output _ 1 = Filtering(num,den,c1,Time,FCornerFilter,Filter_TNoise);
Filter_Output _ 2 = Filtering(num,den,c2,Time,FCornerFilter,Filter_TNoise);

[Filter_ 1_Spectrum, Filter_ 1_f _TS, PSD_Filter _ 1, Filter_ 1_f _OS, Filter_ 1_Window] = wall_fresp(Filter_Output _ 1, Time, @rectwin, 0);
% [snr5, enob5, pot_signal _B5, f5, PSD5] = gs_fresp(Filter_Output _ 2(length (Time)/2 +1:end)', NumSamples/2, fs, f_in, 1);
...
clf
subplot(2,1,1)
plot(Time,Filter_Output _ 1)
xlabel('Time')
ylabel('Filter Output (c1)')
title('Filter output')
subplot(2,1,2)
semilogx(Filter_ 1_f _OS,PSD_Filter _ 1)
...
Mixer_Output = Mixer(Filter_Output _ 1,Filter_Output _ 2,TNoise_Mixer,FCorner_Mixer);
[M_Spectrum, M_f _TS, PSD_M, M_f _OS, M_Window] = wall_fresp(Mixer_Output, Time, @rectwin, 0);
...
filter_ 2 = tf(num1,den1);

% Mixer output after filtering
Final_Output = Filtering(num1,den1,Mixer_Output,Time,FCornerFilter,Filter_TNoise);

[Final_Spectrum, Final_f _TS, PSD_Final, Final_f _OS, Final_Window] = wall_fresp(Final_Output, Time, @rectwin, 0);
...
title('Final output')
subplot(2,1,2)
semilogx(Final_f _OS,PSD_Final)
